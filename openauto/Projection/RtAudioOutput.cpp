...
bool RtAudioOutput::open()
{
    ...
    if(dac_->getDeviceCount() > 0)
    {
        ...
        {
            OPENAUTO_LOG(error) << "[RtAudioOutput] Failed to open audio output, what: " << e;
        }
    }
    else
    {
...
void RtAudioOutput::start()
{
    ...
    if(dac_->isStreamOpen() && !dac_->isStreamRunning())
    {
        ...
        {
            OPENAUTO_LOG(error) << "[RtAudioOutput] Failed to start audio output, what: " << e;
        }
    }
}
...
void RtAudioOutput::doSuspend()
{
    if(dac_->isStreamOpen() && dac_->isStreamRunning())
    {
        ...
        {
            OPENAUTO_LOG(error) << "[RtAudioOutput] Failed to suspend audio output, what: " << e;
        }
    }
}
